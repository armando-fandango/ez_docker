...
create_venv () {
  ...
  mamba create -y -p $venv -c conda-forge "python=$py_ver"
  return $?
}
...
ez_create_env () {
  venv=${venv:-$(mamba info --base)/envs/ez}
  ...
  install_cuda && install_pytorch && install_yml && install_jupyter && config_jupyter
  install_result="$?"
  if [ "$install_result" != "0" ];
  then
...
ez_install_mamba () {
  ...
  #old : https://github.com/conda-forge/miniforge/releases/download/4.12.0-3/Mambaforge-4.12.0-3-Linux-x86_64.sh
  wget -nv https://github.com/conda-forge/miniforge/releases/download/23.1.0-1/Mambaforge-23.1.0-1-Linux-x86_64.sh -O mamba.sh && \
